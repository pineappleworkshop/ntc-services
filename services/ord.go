...
func (tool *InscriptionTool) _initTool(net *chaincfg.Params, request *InscriptionRequest) error {
	...
	if err = tool.buildCommitTx(request.CommitTxOutPointList, totalRevealPrevOutput, request.CommitFeeRate); err != nil {
		return err
	}
...
func (tool *InscriptionTool) BuildInscribe() (commitTxHash *wire.MsgTx, revealTxHashList []*wire.MsgTx, fees int64, err error) {
	fees = tool.calculateFee()

	return tool.commitTx, tool.revealTx, fees, nil
}
